...
3.  **Обработка каждого файла**:
    ...
    *   **Конвертация стоимости**: В тысячах USD (стандарт проекта). До 2025-09 сырые данные в миллионах → `STOIM` умножается на 1000. С 2025-09 источник (tradestat) отдаёт значения уже в тысячах → умножение не применяется.
