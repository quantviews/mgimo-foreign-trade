...
3.  **Обработка каждого файла**:
    ...
    *   **Конвертация стоимости**: Итоговый `STOIM` — в USD (стандарт единой базы, как у CN/TR). Сначала сырые значения MEIDB приводятся к тысячам USD: по эвристике суммы файла (`infer_india_stoim_multiplier`) значения в млн USD умножаются на 1000, значения уже в тыс. USD не трогаются. Затем результат умножается ещё на 1000 → USD.
...
5.  **Финализация**:
    ...
    *   Выход проверяется в рантайме через `assert_country_output_contract()`.
