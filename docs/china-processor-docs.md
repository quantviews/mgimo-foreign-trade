...
3.  **Обработка каждого файла**:
    ...
    *   **Стандартизация**: Коды ТН ВЭД (`TNVED`, `TNVED2`, `TNVED4`, `TNVED6`) приводятся к строковому формату, сохраняя ведущие нули источника.
    *   **Обогащение данных**:
    ...
    *   **Приведение к модели**: Данные приводятся в соответствие с общей моделью через `finalize_country_output()`: добавляется колонка `STRANA` со значением `CN`, форматируется колонка `PERIOD`, генерируются производные `TNVED*`.
