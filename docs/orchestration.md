...
Скачивание месячных parquet и сборка `db/comtrade.db` — два шага flow перед
merge. Оба opt-in: первый ходит в сеть и тратит квоту API, второй перечитывает
весь каталог parquet.

```bash
# что устарело — без ключа, без загрузки, без расхода квоты
python src/collectors/comtrade_collector.py --check

# догрузить недостающее и обновить пересмотренное, не больше 130 запросов
python src/collectors/comtrade_collector.py --refresh --max-requests 130
```

Существенно для планирования refresh: месяц Comtrade нельзя считать готовым
после первой загрузки. Страны досылают и правят отчётность годами — на июнь
2025 отчётность пересмотрел 51 репортёр из 95 уже после того, как файл был
скачан. Сборщик ведёт манифест с `datasetChecksum` по каждой стране и
перекачивает только изменившееся; на бесплатном тарифе прогон ограничивается
через `--max-requests` и возобновляется со следующего запуска (код возврата 2).
...
- отсутствие наименований ТН ВЭД, где кириллица смыкается с латиницей внутри слова
  (`ПОТASSIUM МЕТАБИСУЛЬФИТ`) — след машинного перевода, оборвавшегося на полуслове.
  Валит сборку только для наименований, которые производим мы сами (`NAME_SOURCE`
  `mt` или `manual`); латинские двойники кириллических букв в самом справочнике ФТС
  (`ДИЭЛЕКТPИЧЕСКИХ`, `CЕЙФЫ`) — дефект исходника и отдаются метрикой
  `mixed_script_official_names`, а не отказом.
