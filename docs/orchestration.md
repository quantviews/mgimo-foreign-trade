...
# плюс перекачать пересмотренные страны, не больше 150 запросов за прогон
python src/collectors/comtrade_collector.py --refresh --max-requests 150
```
...
### Расход запросов на бесплатном тарифе

Бесплатная подписка ограничена числом обращений в сутки, поэтому запросы
группируются. API принимает списки и в `reporterCode`, и в `flowCode`, так что
одна пачка стран запрашивается сразу по экспорту и импорту:

| стран в запросе | запросов на полный догон |
|---|---|
| 1 страна, 1 поток (как в исходном скрипте) | 4 142 |
| 1 страна, оба потока | 2 071 |
| 4 страны | 554 |
| **8 стран (по умолчанию)** | **301** |

`--max-requests N` останавливает прогон, когда бюджет исчерпан: манифест
дописывается, код возврата 2, следующий запуск продолжает с того же места.
Порядок обхода — сначала недостающие месяцы, затем самые свежие, чтобы
ограниченный бюджет тратился на то, что нужнее. Полный догон по годам при
пачке по 8: 2025 — 90 запросов, 2024 — 41, 2023 — 32, остальные годы по 24-25.

### Обрезка ответа

`maxRecords` — это наш параметр, а не предел сервера, и он режет ответ молча:
в теле нет признака обрезки, а поле `count` равно числу возвращённых строк, а
не настоящему. Исходный скрипт запрашивал с `maxRecords=50000`, и экспорт
Германии упирался ровно в 50 000 строк в 38 парах «страна-месяц», теряя по
7-11 тысяч записей (2020-02: в файле 50 000, на самом деле 61 078).

Сейчас лимит поднят до 250 000, а совпадение длины ответа с лимитом считается
признаком обрезки: пачка делится пополам и запрашивается заново. Пачку из одной
страны делить некуда — такой случай остаётся в логе предупреждением.
