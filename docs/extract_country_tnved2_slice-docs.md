...
Этот документ описывает работу скрипта `extract_country_tnved2_slice.py`, который предназначен для извлечения срезов данных из итоговой базы данных по стране и коду ТН ВЭД (2- или 4-значный), с опциональной фильтрацией по году и поддержкой оригинальных наименований по странам (Турция, Китай, Индия).
...
2. **Построение SQL-запроса**: 
   - Базовый запрос выбирает все данные из представления `unified_trade_data_enriched`, которое включает:
     ...
     - Названия кодов TNVED разных уровней (`TNVED2_NAME`, `TNVED4_NAME`, `TNVED6_NAME`, `TNVED8_NAME`, `TNVED_NAME`),
       их происхождение (`TNVED_NAME_SOURCE`), английский оригинал (`TNVED_NAME_EN`),
       ближайшее официальное наименование (`TNVED_NAME_OFFICIAL`) и единица измерения (`TNVED_UNIT`)
...
4. **Добавление оригинальных названий по странам (опционально)**:
   - Для каждой страны из выборки (заданной через `--country` или присутствующей в данных при выгрузке «все страны») скрипт проверяет, поддерживается ли загрузка оригинальных наименований. Поддерживаются: **TR** (Турция), **CN** (Китай), **IN** (Индия). Используется функция `load_country_original_names`.
   - Источники данных по странам:
     - **TR**: директория `data_raw/turkey/hs_codes_json/`, файлы `turkey_codes*.json` (словарь HS8 → название).
     - **CN**: директория `metadata/china/`, файлы `*-codes.json` (список записей с полями `TNVED` и `COMMODITY_NAME`).
     - **IN**: директория `data_raw/india_new/`, файлы `india_*.csv` (колонки `TNVED`, `Commodity`).
   - Для каждой строки по 10‑значному коду `TNVED` берутся первые 8 знаков (HS8); по ним выполняется поиск оригинального названия для страны, соответствующей колонке `STRANA`.
   - Добавляются колонки (по одной на поддерживаемую страну, попавшую в выборку):
     - `TNVED_NAME_ORIGINAL_TR`, `TNVED_NAME_ORIGINAL_CN`, `TNVED_NAME_ORIGINAL_IN` — оригинальное наименование для данной страны (заполняется только в строках, где `STRANA` совпадает с кодом страны; в остальных — пустая строка).

5. **Сохранение результатов**:
   - В зависимости от режима работы создаются один или несколько файлов в формате **CSV** или **Excel**:
     - **Режим 1 (одна страна + код)**: указаны один `--country` и ( `--tnved4` **или** `--tnved2` ) → создается один файл:
       - при `--tnved4`: `{COUNTRY}_TNVED4_{CODE4}[_from_YEAR].{ext}`
       - при `--tnved2`: `{COUNTRY}_TNVED2_{CODE2}[_from_YEAR].{ext}`
     - **Режим 2 (несколько стран + код)**: указаны несколько `--country` и ( `--tnved4` **или** `--tnved2` ) → создается отдельный файл для каждой страны:
       - `{COUNTRY1}_TNVED2_{CODE2}[_from_YEAR].{ext}`, `{COUNTRY2}_TNVED2_{CODE2}[_from_YEAR].{ext}`, и т.д.
     - **Режим 3 (одна страна, без кода)**: указана только одна `--country` → создается отдельный файл для каждого кода `TNVED2`:
       - `{COUNTRY}_TNVED2_{CODE2}.{ext}`
     - **Режим 4 (один код, без страны)**: указан только `--tnved2` → создается отдельный файл для каждой страны:
       - `{COUNTRY}_TNVED2_{CODE2}.{ext}`
     - **Режим 5 (ни страна, ни код не указаны)**: создается отдельный файл для каждой комбинации страна-код `TNVED2`:
       - `{COUNTRY}_TNVED2_{CODE2}.{ext}`
...
*   `--country <ISO-код> [ISO-код ...]` (опциональный): Один или несколько двухбуквенных ISO-кодов стран для фильтрации (например, `CN`, или `CN IN TR`). Можно указать несколько стран через пробел. Если не указан, извлекаются данные для всех стран.
...
```bash
python src/extract_country_tnved2_slice.py --country TR --tnved 2710 --year 2021
```
...
**5. Извлечь данные для нескольких стран по одному коду:**
```bash
python src/extract_country_tnved2_slice.py --country CN IN TR --tnved2 27
```
Создаст отдельные CSV файлы для каждой из указанных стран по коду 27: `CN_TNVED2_27.csv`, `IN_TNVED2_27.csv`, `TR_TNVED2_27.csv`.

**6. Извлечь все комбинации (создаст отдельные файлы для каждой пары страна-код):**
```bash
...
**7. Использование с указанием путей и формата:**
```bash
python src/extract_country_tnved2_slice.py \
  --country TR \
  --tnved2 87 \
...
*   `TNVED_NAME_OFFICIAL` — ближайшее наименование из официального справочника, и
    `TNVED_NAME_OFFICIAL_LEVEL` — уровень, на котором оно нашлось. Нужны там, где на
    10 знаках лежит машинный перевод, а официальный текст есть только на 8 или 6
*   `TNVED_NAME_EN` — английский оригинал, из которого сделан машинный перевод
*   `TNVED_NAME_SOURCE` — происхождение наименования: `fts` / `fns` (официальные справочники),
    `manual` (выверено вручную), `mt` (машинный перевод)
*   `TNVED_UNIT` — дополнительная единица измерения, отделённая от текста наименования
*   `period_rank` — порядковый номер записи по периоду для каждой страны (1 = самый последний месяц)

**Дополнительные колонки (оригинальные названия по странам):**
*   `TNVED_NAME_ORIGINAL_TR` — оригинальное наименование для Турции (HS8 из `data_raw/turkey/hs_codes_json/`).
*   `TNVED_NAME_ORIGINAL_CN` — оригинальное наименование для Китая (из `metadata/china/*-codes.json`).
*   `TNVED_NAME_ORIGINAL_IN` — оригинальное наименование для Индии (из `data_raw/india_new/india_*.csv`).

Колонки добавляются только для тех стран (TR, CN, IN), которые попали в выборку (указаны в `--country` или присутствуют в данных при выгрузке без фильтра по стране). В каждой строке заполняется только колонка той страны, которая указана в `STRANA`; остальные ячейки в этих колонках — пустые.
