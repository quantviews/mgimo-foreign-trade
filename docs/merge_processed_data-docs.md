...
Скрипт `merge_processed_data.py` предназначен для объединения всех обработанных наборов данных (в формате `.parquet` из папки `data_processed/`) в единую базу данных DuckDB. Он также позволяет опционально включать данные из базы Comtrade для стран, по которым отсутствуют национальные данные, **по умолчанию подмешивает nowcast** из `data_processed/nowcast/nowcast.parquet` (только строки с `TYPE='pred'`); предикты по ячейкам, где уже есть факт, **удаляются** при merge. Предусмотрены гибкие опции фильтрации и выбор целевого DuckDB-файла через `--output-db-path`.
...
*   первый merge строит базу из фактических источников; если `run_nowcast=True`, nowcast в первый merge не включается, чтобы R-скрипт не читал старые прогнозные строки;
...
2.  **Загрузка и валидация**: Каждый найденный `.parquet` файл загружается и проходит строгую проверку:
    ...
    *   **Fizob файлы**: Файлы с расчетными физическими объемами, начинающиеся с `fizob` (например, `fizob2.parquet`, `fizob4.parquet`, `fizob6.parquet`), обрабатываются отдельно и приводятся к унифицированной структуре для записи в таблицу `fizob_index`. Для них пропускается строгая проверка `EXPECTED_SCHEMA`, так как у fizob-данных другая исходная форма.
