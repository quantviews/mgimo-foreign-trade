...
2.  **Загрузка и валидация**: Каждый найденный `.parquet` файл загружается и проходит строгую проверку:
    ...
    *   **Генерация производных колонок**: Скрипт автоматически генерирует колонки `TNVED2`, `TNVED4`, `TNVED6`, `TNVED8` на основе основной колонки `TNVED`. 
        ...
        *   **Нормализация кодов TNVED**: После генерации производных колонок основной код `TNVED` нормализуется до единой длины:
...
3.  **Интеграция данных Comtrade (опционально)**:
    ...
    *   **Трансформация данных**: Данные из Comtrade преобразуются в соответствии с моделью данных проекта:
        ...
        *   Генерируются производные колонки `TNVED2`, `TNVED4`, `TNVED6`, `TNVED8` на основе `TNVED`. **Важно**: Производные колонки генерируются ДО нормализации, чтобы сохранить правильную структуру для кодов 01–10. Затем коды TNVED нормализуются так же, как и для национальных данных: дополняются нулями справа до 10 знаков (если короче); ведущие нули **не удаляются**.
...
8.  **Сохранение результата**:
    ...
    *   Запись выполняется **атомарно**: данные сначала записываются во временный файл `unified_trade_data.duckdb.tmp`, и только после успешного завершения записи файл заменяет целевую базу данных (`os.replace()`). При ошибке временный файл удаляется, существующая база остаётся нетронутой.
    *   Если при запуске остался файл `.tmp` от предыдущей упавшей сборки, он автоматически удаляется перед началом записи.
