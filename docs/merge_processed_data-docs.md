...
*   `build_merged_dataframe()` — финальное объединение, фильтрация, нормализация `EDIZM` и спецправила `KG`/`TONNE`/`BQ`.
...
*   **`unified_trade_data`** — основная таблица с данными о внешней торговле (без названий стран и TNVED для экономии места). Колонки `SOURCE` и `TYPE` позволяют отделить национальные/Comtrade данные от nowcast и факт от прогноза (`pred`).
