...
2.  **Загрузка и валидация**: Каждый найденный `.parquet` файл загружается и проходит строгую проверку:
    ...
    *   **Генерация производных колонок**: Скрипт автоматически генерирует колонки `TNVED2`, `TNVED4`, `TNVED6`, `TNVED8` на основе основной колонки `TNVED`. Колонка `TNVED` дополняется до минимум 8 символов (с помощью `zfill(8)`), после чего производные колонки вычисляются как срезы строки соответствующей длины.
...
3.  **Интеграция данных Comtrade (опционально)**:
    ...
    *   **Трансформация данных**: Данные из Comtrade преобразуются в соответствии с моделью данных проекта:
        ...
        *   Генерируются производные колонки `TNVED2`, `TNVED4`, `TNVED6`, `TNVED8` на основе `TNVED`. Колонка `TNVED` дополняется до минимум 8 символов перед вычислением производных колонок.
...
8.  **Создание справочных таблиц**:
    *   **Нормализованная структура**: Вместо добавления названий как столбцов в основную таблицу, скрипт создает отдельные справочные таблицы для нормализации структуры базы данных:
...
*   **`unified_trade_data_enriched`** — представление (VIEW), которое объединяет основную таблицу со справочными таблицами, добавляя все названия. Это удобно для использования в Superset, так как не требует ручного создания JOIN'ов.
...
10. **Вывод статистики**: В процессе работы скрипт выводит в консоль подробную статистику о результатах объединения, включая:
...
*   **Нормализованная структура базы данных**: Справочные данные (названия стран и TNVED) хранятся в отдельных таблицах, что:
    *   Значительно уменьшает размер основной таблицы (названия не дублируются в каждой строке)
