...
3.  **Последовательные запросы**: Сбор данных для экспорта (`Ex`) и импорта (`Im`) выполняется последовательно, с паузами между запросами, чтобы не перегружать MEIDB.
...
7.  **Постобработка**: К финальному набору данных применяются следующие преобразования:
    *   Направление торговли инвертируется относительно индийского источника (`Ex` → `ИМ`, `Im` → `ЭК`) через единую точку `mirror_napr_value()` из `core/country_processor_contract.py`.
    ...
    *   **Масштаб `STOIM` collector не меняет** — приведение к тысячам USD выполняется в `india_processor.py` (см. `docs/india-processor-docs.md`).
8.  **Проверка публикации месяца**: Перед сохранением вызывается `has_published_trade_data()`. Если `STOIM.sum() == 0` и `KOL.sum() == 0`, месяц **не записывается** — MEIDB иногда отдаёт «скелет» таблицы для ещё не опубликованных периодов.
9.  **Сохранение данных**: Обработанный `DataFrame` сохраняется в файл формата `.csv` в директорию `data_raw/india_new/` с именем `india_ГОД_МЕСЯЦ.csv`.
...
1.  **Сбор данных за конкретный месяц (например, Август 2025):**
    ...
    ```bash
    python india_collector.py --year 2026 --month 4
    ```
