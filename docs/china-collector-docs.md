# Документация для `china_collector.py`

Этот документ описывает работу скрипта `china_collector.py` (до 2026-07 — `china-collector.py`), его полуавтоматическую логику и способы использования для сбора данных о внешней торговле Китая.

## Описание

Скрипт `china_collector.py` предназначен для сбора данных о внешней торговле Китая с официального портала Таможенной службы Китая (`http://stats.customs.gov.cn/indexEn`).
...
5.  **Обработка данных**: Если все проверки пройдены, скрипт:
    ...
    *   "Отзеркаливает" направление торговли (`NAPR`) для соответствия российской перспективе через `mirror_napr_value()` из `core/country_processor_contract.py` (китайский импорт становится экспортом для России и наоборот).
    *   Приводит `TNVED` к 8-значному коду с ведущими нулями. Производные `TNVED2/4/6` в raw-файл не пишутся — их выводит `finalize_country_output()` на этапе процессора.
...
2.  Запустите скрипт, указав обязательные аргументы: `год`, `месяц` и `направление`.
    ...
    ```bash
    python src/collectors/china_collector.py <год> <месяц> <направление> [--partner <код_партнера>]
    ```
...
```bash
python src/collectors/china_collector.py 2026 04 ИМ --partner 344
python src/collectors/china_collector.py 2026 04 ЭК --partner 344
```
После выполнения этой команды откроется браузер. Вам нужно будет вручную выбрать на сайте `Year: 2025`, `Month: August`, `I/E: Import`, `Partner: Russia`, `By month:` (галочка),   решить CAPTCHA и скачать файл. Затем вернуться в терминал и нажать `Enter`. В результате будет создан файл в `data_raw/china/EXPORT/`, так как импорт для Китая является экспортом для России.
