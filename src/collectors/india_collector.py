# -*- coding: utf-8 -*-
"""Коллектор данных Индии: выгружает месячные данные MEIDB в data_raw/india_new/."""
...
def post_process_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # Зеркалирование потока страна<->РФ единой точкой контракта:
    # экспорт Индии (Ex) = импорт РФ, импорт Индии (Im) = экспорт РФ.
    df.Flow = df.Flow.apply(lambda x: mirror_napr_value('ЭК' if x == 'Ex' else 'ИМ'))
...
def save_india_data(df: pd.DataFrame, year: int, month: int, output_dir: Optional[Path] = None) -> None:
    ...
    if output_dir is None:
        output_dir = get_project_root() / 'data_raw' / 'india_new'
